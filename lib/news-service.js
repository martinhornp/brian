...
const GOOGLE_FEED_SOURCES = GOOGLE_BACKFILL_QUERIES.map((query, index) => ({
  name: `Google News Backfill ${index + 1}`,
  url: `https://news.google.com/rss/search?q=${encodeURIComponent(query)}&hl=da&gl=DK&ceid=DK:da`,
}));
