...
const BING_MARKETS = [
  { label: "DK", cc: "dk", lang: "da-dk", siteFilter: "site:.dk" },
  { label: "SE", cc: "se", lang: "sv-se", siteFilter: "site:.se" },
  { label: "NO", cc: "no", lang: "no-no", siteFilter: "site:.no" },
];
...
const GOOGLE_MARKETS = [
  { label: "DK", hl: "da", gl: "DK", ceid: "DK:da" },
  { label: "SE", hl: "sv", gl: "SE", ceid: "SE:sv" },
  { label: "NO", hl: "no", gl: "NO", ceid: "NO:no" },
];
...
const BING_FEED_SOURCES = BING_MARKETS.flatMap((market) =>
  BING_KEYWORDS.flatMap((keyword) =>
    BING_OFFSETS.map((offset) => {
      const query = encodeURIComponent(`"${keyword}" ${market.siteFilter}`);
      const offsetPart = offset > 0 ? `&first=${offset}` : "";

      return {
        name: `Bing ${market.label} - ${keyword} (${offset || "base"})`,
        url: `https://www.bing.com/news/search?q=${query}&format=rss&cc=${market.cc}&setlang=${market.lang}${offsetPart}`,
      };
    })
  )
);

const GOOGLE_FEED_SOURCES = GOOGLE_MARKETS.flatMap((market) =>
  GOOGLE_BACKFILL_QUERIES.map((query, index) => ({
    name: `Google ${market.label} Backfill ${index + 1}`,
    url: `https://news.google.com/rss/search?q=${encodeURIComponent(query)}&hl=${market.hl}&gl=${market.gl}&ceid=${market.ceid}`,
  }))
);

const FEED_SOURCES = [...CORE_FEED_SOURCES, ...BING_FEED_SOURCES, ...GOOGLE_FEED_SOURCES];
...
function isLikelyNordicNews(item) {
  const source = normalizeWhitespace(item.source).toLowerCase();

  try {
    const host = new URL(item.link).hostname.toLowerCase();
    if (host.endsWith(".dk") || host.endsWith(".se") || host.endsWith(".no")) {
      return true;
    }
    if (host.includes(".dk") || host.includes(".se") || host.includes(".no")) {
      return true;
    }
  } catch {
  ...
  return NORDIC_SOURCE_HINTS.some((hint) => source.includes(hint));
}
...
async function crawlSourceWithTimeout(source) {
  let timeoutId;
  ...
  try {
    return await Promise.race([crawlSource(source), timeoutPromise]);
  } finally {
    if (timeoutId) {
...
async function crawlAllFeeds() {
  ...
  const nordicMatches = matches.filter(isLikelyNordicNews);
  const dedupedMatches = deduplicate(nordicMatches);
